  ...
  h.onMessage([&lane, &ref_vel, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
		...
		if (prev_size <2)
		/* If the list is small just create a previous pt by extrpolating *
		 * and taking a time step back                                    */
		{
		...
		{
		    /* Push the last 2 points from the previous path */
		...
		/* Convert coordinate to the origin of current car position */
		...
		double target_x = 30.0; // Split remaining points into equally spaced points for 30 meter
